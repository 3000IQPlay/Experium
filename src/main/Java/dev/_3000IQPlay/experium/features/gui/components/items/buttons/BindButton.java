...
public class BindButton
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (ClickGui.getInstance().rainbowRolling.getValue().booleanValue()) {
            ...
            RenderUtil.drawRect(this.x, this.y, this.x + (float)this.width + 7.4f, this.y + (float)this.height - 0.5f, this.getState() ? (!this.isHovering(mouseX, mouseY) ? Experium.colorManager.getColorWithAlpha(((ClickGui)Experium.moduleManager.getModuleByName((String)"ClickGui")).moduleMainC.getValue().getAlpha()) : Experium.colorManager.getColorWithAlpha(((ClickGui)Experium.moduleManager.getModuleByName((String)"ClickGui")).hoverAlpha.getValue())) : (!this.isHovering(mouseX, mouseY) ? 0x11555555 : -2007673515));
        }
        if (this.isListening) {
