...
public class AutoCrystal
    ...
    public Setting<Integer> placeDelay = this.register(new Setting<Object>("PlaceDelay", 10, 0, 500, v -> this.setting.getValue() == Settings.PLACE && this.place.getValue()));
    ...
    public Setting<Boolean> manual = this.register(new Setting<Object>("Manual", Boolean.FALSE, v -> this.setting.getValue() == Settings.BREAK));
    ...
    private final Setting<Float> moveSpeed = this.register(new Setting<>("Speed", 1000.0f, 0.0f, 1500.0f, v -> this.setting.getValue() == Settings.RENDER && this.renderMode.getValue() == RenderMode.GLIDE && this.render.getValue()));
    ...
    private final Setting<Integer> bRed = this.register(new Setting<Object>("BoxRed", 100, 0, 255, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.box.getValue()));
    ...
    private final Setting<Integer> bBlue = this.register(new Setting<Object>("BoxBlue", 240, 0, 255, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.box.getValue()));
    ...
    private final Setting<Integer> oRed = this.register(new Setting<Object>("OutlineRed", 100, 0, 255, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.outline.getValue()));
    private final Setting<Integer> oGreen = this.register(new Setting<Object>("OutlineGreen", 0, 0, 255, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.outline.getValue()));
    private final Setting<Integer> oBlue = this.register(new Setting<Object>("OutlineBlue", 240, 0, 255, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.outline.getValue()));
    ...
    public Setting<AutoSwitch> autoSwitch = this.register(new Setting<Object>("Switch", AutoSwitch.SILENT, v -> this.setting.getValue() == Settings.MISC));
            ...
            if (packet2.getType() == 51 && AutoCrystal.mc.player.getDistanceSq(pos = new BlockPos(packet2.getX(), packet2.getY(), packet2.getZ())) + (double) this.predictOffset.getValue() <= MathUtil.square(this.breakRange.getValue()) && (this.instantTimer.getValue() == PredictTimer.NONE || this.instantTimer.getValue() == PredictTimer.BREAK && this.breakTimer.passedMs(this.breakDelay.getValue()) || this.instantTimer.getValue() == PredictTimer.PREDICT && this.predictTimer.passedMs(this.predictDelay.getValue()))) {
                ...
                if (this.predictFriendDmg.getValue() && (this.antiFriendPop.getValue() == AntiFriendPop.BREAK || this.antiFriendPop.getValue() == AntiFriendPop.ALL) && this.isRightThread()) {
                    for (EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        if (friend == null || AutoCrystal.mc.player.equals(friend) || friend.getDistanceSq(pos) > MathUtil.square(this.range.getValue() + this.placeRange.getValue()) || !Experium.friendManager.isFriend(friend) || !((double) DamageUtil.calculateDamage(pos, friend) > (double) EntityUtil.getHealth(friend) + 0.5))
                            continue;
                        return;
                    }
                }
        ...
        for (Entity entity : AutoCrystal.mc.world.loadedEntityList) {
            ...
            if ((double) selfDamage + 0.5 < (double) EntityUtil.getHealth(AutoCrystal.mc.player) && selfDamage <= this.maxSelfBreak.getValue()) {
                ...
                for (EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                    ...
                    if (this.antiFriendPop.getValue() != AntiFriendPop.BREAK && this.antiFriendPop.getValue() != AntiFriendPop.ALL || !Experium.friendManager.isFriend(player.getName()) || !((double) DamageUtil.calculateDamage(entity, player) > (double) EntityUtil.getHealth(player) + 0.5))
                        continue;
