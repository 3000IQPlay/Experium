...
import dev._3000IQPlay.experium.mixin.mixins.accessors.IEntityLivingBase;
...
class EntityUtil implements Util {
    ...
    public static List<Vec3d> getUnsafeBlocks(final Entity entity, final int height, final boolean floor, final boolean face) {
        return getUnsafeBlocksFromVec3d(entity.getPositionVector().add(0, 0.125, 0), height, floor, face);
    }
    ...
    public static List<EntityPlayer> getNearbyPlayers(double d) {
        ...
        list.removeIf(entityPlayer -> Experium.friendManager.isFriend(entityPlayer.getName()));
        return list;
    }
    ...
    public static boolean isntValid(final Entity entity, final double range) {
        return entity == null || isDead(entity) || entity.equals(EntityUtil.mc.player) || (entity instanceof EntityPlayer && Experium.friendManager.isFriend(entity.getName())) || EntityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(final Entity entity, final int red, final int green, final int blue, final int alpha, final boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer) {
            if (colorFriends && Experium.friendManager.isFriend((EntityPlayer) entity)) {
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (final EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            ...
            output.put(healthSB + " " + (Experium.friendManager.isFriend(player) ? "\u00A7b" : "\u00A7r") + player.getName() + " " + distanceSB + " " + "\u00A7f" + Experium.totemPopManager.getTotemPopString(player) + Experium.potionManager.getTextRadarPotion(player), (int) EntityUtil.mc.player.getDistance(player));
