...
public class RenderUtil
    ...
    void drawSexyBox(AxisAlignedBB a, Color boxColor, Color outlineColor, float lineWidth, boolean outline, boolean box, boolean colorSync, float alpha, float scale, float slab) {
    ...
    public static void drawText(AxisAlignedBB pos, String text) {
        ...
        GlStateManager.disableDepth();
        GlStateManager.translate(-((double) Experium.textManager.getStringWidth(text) / 2.0), 0.0, 0.0);
        Experium.textManager.drawStringWithShadow(text, 0.0f, 0.0f, -5592406);
        GlStateManager.popMatrix();
    }
    ...
    public static void drawProjectedText(GLUProjection.Projection projection, float addX, float addY, String text, Color color, boolean shadow) {
        if (projection.getType() == GLUProjection.Projection.Type.INSIDE) {
            GlStateManager.pushMatrix();
            GlStateManager.translate(projection.getX(), projection.getY(), 0.0);
            Experium.textManager.drawString(text, -((float) Experium.textManager.getStringWidth(text) / 2.0f) + addX, addY, color.getRGB(), shadow);
            GlStateManager.translate(-projection.getX(), -projection.getY(), 0.0);
            GlStateManager.popMatrix();
        }
    }
